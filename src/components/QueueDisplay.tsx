...
export const QueueDisplay = () => {
  const { departments, getWaitingPatients, getQueueStats } = useQueue();
  ...
  const DepartmentCard = ({ department }: { department: any }) => {
    const waitingPatients = getWaitingPatients(department.name);
    const emergencyCount = waitingPatients.filter(p => p.isEmergency).length;
    const seniorCount = waitingPatients.filter(p => p.isSeniorCitizen && !p.isEmergency).length;
                ...
                <Card className="mt-6">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {getWaitingPatients(department.name).slice(0, 10).map((patient, index) => (
                        <div 
                          key={patient.id} 
                          className={`flex items-center justify-between p-3 rounded-lg ${
                            patient.isEmergency ? 'bg-destructive/10 border border-destructive/20' :
                            patient.isSeniorCitizen ? 'bg-priority/10 border border-priority/20' :
                            'bg-muted/30'
                          }`}
                        >
                          ...
                          <div className="text-right">
                            <div className="text-sm text-muted-foreground">
                              {patient.registrationTime.toLocaleTimeString()}
                            </div>
                            {patient.isEmergency && (
                      ...
                      {getWaitingPatients(department.name).length === 0 && (
